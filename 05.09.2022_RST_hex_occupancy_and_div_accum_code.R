...
#create csv file for use with the "beta diversity street tree code" script
...
#check trees per hex- 
numberhex_per_holcid <- tabyl(env,holc_id, show_na = FALSE)
...
ggsave(file=sprintf("output/tree_number_grade_hex_live%s.pdf",date), plot=tree_number_grade_hex_live, width=6, height=4)
...
tree_number_grade_hex_empty<-ggplot(env, aes(x=holc_grade, y=proportion_of_possible_sites_empty,fill=holc_grade))+ geom_jitter(width = 0.2, colour="black",alpha=.2)+
  geom_boxplot(notch=TRUE,outlier.shape = NA,weight=5)+scale_fill_manual(values = holc_col)+
  labs(x = "HOLC Grade", y = "proportion of viable sites in hex occupied", fill = "HOLC grade", shape = "HOLC grade")+ theme(legend.position = "none")
...
tree_number_grade_hex_small<-ggplot(env, aes(x=holc_grade, y=proportion_of_possible_sites_occupied_by_small,fill=holc_grade))+ geom_jitter(width = 0.2, colour="black",alpha=.2)+
  geom_boxplot(notch=TRUE,outlier.shape = NA,weight=5)+scale_fill_manual(values = holc_col)+
  labs(x = "HOLC Grade", y = "proportion of viable sites in hex occupied", colour = "HOLC grade", shape = "HOLC grade")+ theme(legend.position = "none")
...
qlabels <- c("0" = "richness (q=0)", "1" = "Shannon's ESN (q=1)","2" = "Simpson's ESN (q=2)")
...
accum_alltree_all_q<-ggiNEXT(out.all, type=1, facet.var="order") + theme_bw(base_size=10)+ theme_bw(base_size=10)+ xlim(c(0,20000))+scale_colour_manual(values = holc_col,name="HOLC grade")+scale_fill_manual(values = holc_col, name="HOLC grade")+
  labs(x = "Number of individual trees sampled", y = "street tree diversity", colour = "HOLC grade", shape = "HOLC grade", fill="HOLC grade")+
  theme(legend.position="none",legend.title=element_blank())+facet_wrap(~order, scales="free",labeller=labeller(order = qlabels))

#save summary figure

ggsave(file=sprintf("output/accum_alltree_all_div_pooled_indv%s.pdf",date), plot=accum_alltree_all_q, width=7, height=4)
ggsave(file=sprintf("output/accum_alltree_all_div_pooled_indv%s.tiff",date), plot=accum_alltree_all_q, width=7, height=4,dpi=300, compression = "lzw")
...
density_DBH<-ggplot(st_tree2,aes(DBH,fill = holc_grade, colour = holc_grade))+
  geom_density(alpha = 0.1) +
  xlim(0, 50)+theme_bw(base_size=10)+scale_colour_manual(values = holc_col)+scale_fill_manual(values = holc_col)

freqpoly_DBH<-ggplot(st_tree2,aes(DBH,fill = holc_grade, colour = holc_grade)) +
  geom_freqpoly(binwidth = 1)+
  xlim(0, 50)+theme_bw(base_size=10)+scale_colour_manual(values = holc_col)+scale_fill_manual(values = holc_col)

#save summary figure
ggsave(file=sprintf("output/density_DBH%s.pdf",date), plot=density_DBH, width=5, height=2.3)
ggsave(file=sprintf("output/density_DBH%s.tiff",date), plot=density_DBH, width=5, height=2.3)

ggsave(file=sprintf("output/freqpoly_DBH%s.pdf",date), freqpoly_DBH, width=5, height=3)
ggsave(file=sprintf("output/freqpoly_DBH%s.tiff",date), freqpoly_DBH, width=5, height=3)


density_DBH<-ggplot(st_tree2,aes(DBH,fill = holc_grade, colour = holc_grade))+
  geom_density(alpha = 0.1) +
  xlim(0, 50)+ scale_colour_manual(values = holc_col)+scale_fill_manual(values = holc_col)+ theme_bw(base_size=10)+ theme_bw(base_size=10)+
  labs(x = "DBH (tree diameter at 1.37m high: inches)", y = "density", colour = "HOLC grade", fill="HOLC grade")+
  theme(legend.position="right",legend.title=element_blank())

density_DBH<-ggplot(st_tree2,aes(DBH,fill = holc_grade, colour = holc_grade))+
  geom_density(alpha = 0.1)+
  ylim(0, 0.15) + xlim(0, 50)+ scale_colour_manual(values = holc_col)+scale_fill_manual(values = holc_col)+ theme_bw(base_size=10)+ theme_bw(base_size=10)+
  labs(x = "DBH (tree diameter at 1.37m high: inches)", y = "density", colour = "HOLC grade", fill="HOLC grade")+
  theme(legend.position="none")+geom_vline(xintercept = 5, linetype="dashed")+geom_vline(xintercept = 20, linetype="dashed")
